...
author: 一泽Eze (OpenClaw adaptation)
---
...
| 场景 | 工具 |
|------|------|
| 搜索摘要或关键词结果，发现信息来源 | **web 搜索**（OpenClaw 内置 Brave Search） |
| URL 已知，需要从页面定向提取特定信息 | **curl** 或 **Jina**（`r.jina.ai/` 前缀转 Markdown，省 token） |
...
浏览器 CDP 不要求 URL 已知——可从任意入口出发，通过页面内搜索、点击、跳转等方式找到目标内容。web 搜索、curl 均不处理登录态。

**Jina**（可选预处理层，可与 curl 组合使用，由于其特性可节省 tokens 消耗，请积极在任务合适时组合使用）：第三方网络服务，可将网页转为 Markdown，大幅节省 token 但可能有信息损耗。调用方式为 `curl -s r.jina.ai/example.com`（URL 前加前缀，不保留原网址 http 前缀），限 20 RPM。适合文章、博客、文档、PDF 等以正文为核心的页面；对数据面板、商品页等非文章结构页面可能提取到错误区块。
...
| 需要 CDP 浏览器或长时间运行的任务 | 几次 web 搜索 / Jina 就能完成的轻量查询 |
